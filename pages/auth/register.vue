    ...
    <div class="auth-modal__dialog">
      ...
      <div class="auth-card">
        <div class="auth-card__head">
          ...
          <div class="auth-card__sub">Join Bibilive</div>
        </div>
        <div class="auth-card__body">
          <div v-if="error" class="auth-note" style="color:#ff6b6b; margin-bottom:8px;">{{ error }}</div>
          <div v-if="success" class="auth-note" style="color:#4ade80; margin-bottom:8px;">{{ success }}</div>
            ...
            <div class="auth-field">
              ...
              <div class="auth-input">
                <input
                  ...
                  required
                />
                <button type="button" class="eye-btn" @click="showPassword2 = !showPassword2" tabindex="-1">
                  <svg v-if="showPassword2" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2.062 12.348a1 1 0 0 1 0-.696 10.75 10.75 0 0 1 19.876 0 1 1 0 0 1 0 .696 10.75 10.75 0 0 1-19.876 0"/><circle cx="12" cy="12" r="3"/></svg>
                  <svg v-else xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 18-.722-3.25"/><path d="M2 8a10.645 10.645 0 0 0 20 0"/><path d="m20 15-1.726-2.05"/><path d="m4 15 1.726-2.05"/><path d="m9 18 .722-3.25"/></svg>
                </button>
              </div>
            </div>
          ...
          <div class="auth-foot">
            ...
            <NuxtLink class="auth-link" to="/auth/login">Sign in</NuxtLink>
          </div>
        </div>
      </div>
    </div>
...
const { signUp, signInWithDiscord, isLoggedIn, USERNAME_MAX_LEN } = useAuth()
...
.auth-input input {
  ...
  border-radius: 8px;
  border: 1px solid rgba(204, 173, 34, 0.22);
  background: #252525;
  color: #fff;
  outline: none;
  font-family: 'Mozilla Text', sans-serif;
...
.auth-input input:focus { border-color: rgba(204,34,34,.6); background: #2a2a2a; }
...
.auth-link { color: #F5A800; text-decoration: none; font-weight: 700; }
